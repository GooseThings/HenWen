...
def lookup_node(node: str) -> dict:
    ...
    if node == _dvswitch_bridge_node_cached():
        # The DVSwitch bridge node is a private/internal pseudo-node
        # (rxchannel usrp/..., never a real AllStar node) -- it's in
        # neither allmondb nor astdb, so without this it shows up
        # everywhere (recent connections, the map, Status Board) as a bare
        # number with a blank name ("1999 ---"), indistinguishable from a
        # real node astdb just hasn't heard of yet.
        #
        # Rather than a fixed label, show whoever's actually on the bridge
        # right now (or was last heard) -- the bridge node's own "identity"
        # at any moment really is whoever's talking through it. callsign
        # here rides the same callsignLink()/QRZ rendering every other
        # node's callsign already gets on the Status Board, so this needs
        # no separate frontend work. Falls back to the generic label when
        # nothing's been heard yet (fresh boot, or a non-STFU network --
        # see start_dvswitch_caller_poller()'s own docstring for why only
...
# to be per-config. Values match rpt.conf's own rxchannel = usrp/
# 127.0.0.1:34001:32001 -- Asterisk binds *its own* socket on 32001
# (confirmed live via `ss -u -p`: the real bound socket belongs to
# asterisk's pid, not Analog_Bridge) and sends its outgoing audio to 34001,
# so Analog_Bridge needs txPort=32001 (to reach Asterisk's bound socket)
# and rxPort=34001 (to receive what Asterisk sends). The names below name
# which port ASTERISK uses each one for, not Analog_Bridge's own key names
# -- dvswitch/apply.sh maps DVSWITCH_USRP_ASTERISK_TXPORT to Analog_
# Bridge's txPort and DVSWITCH_USRP_ASTERISK_RXPORT to its rxPort. These
# used to be mapped backwards (a bug in apply.sh, not here) -- Analog_
# Bridge started and reported status fine either way since its TLV/remote-
# control ports are separate, but DMR audio silently never reached Asterisk
# because Analog_Bridge could never bind a port Asterisk already held.
# Confirmed against this box's own original (pre-DVSwitch-feature)
# Analog_Bridge.ini, which had these the correct way around.
DVSWITCH_USRP_ASTERISK_RXPORT = 34001   # Asterisk sends here; Analog_Bridge's rxPort
DVSWITCH_USRP_ASTERISK_TXPORT = 32001   # Asterisk listens here; Analog_Bridge's txPort
...
# (DVSWITCH_USRP_ASTERISK_RXPORT above), confirmed against the real file
# DVSwitch's own dvswitch.sh discovers via its getUSRPPort() helper.
DVSWITCH_ABINFO_PATH = f"/tmp/ABInfo_{DVSWITCH_USRP_ASTERISK_RXPORT}.json"
...
@_ttl_cached(30)
def _dvswitch_bridge_node_cached():
    """The enabled bridge node number, or None -- read by lookup_node() on
    every node-display lookup across the board (recent connections, map,
...
def api_dvswitch_status():
    ...
    # Last/current DMR caller -- BrandMeister/STFU only for now, see
    # start_dvswitch_caller_poller()'s own docstring for why. caller_id is
    # None on a fresh boot before any traffic has been seen yet, or for a
    # non-STFU network -- the frontend treats that as "nothing heard yet",
    # not an error.
    with _dvswitch_caller_lock:
        caller = dict(_dvswitch_caller_cache)
    return jsonify({
...
def api_dvswitch_apply_status():
    ...
    needed_unit = "stfu" if (cfg and cfg["dmr_network"] == "brandmeister") else "mmdvm_bridge"
    with _dvswitch_status_lock:
        applied = bool(_dvswitch_status_cache.get("analog_bridge") == "active"
                        and _dvswitch_status_cache.get(needed_unit) == "active")
    return jsonify({
        "ready":              ready,
...
_dvswitch_status_cache = {"analog_bridge": None, "mmdvm_bridge": None, "stfu": None, "checked_at": 0}
...
def start_dvswitch_status_poller():
    def _loop():
        while True:
            try:
                # Polls all three units regardless of which one the current
                # config actually needs -- cheap (one systemctl call) and lets
                # api_dvswitch_apply_status()/diagnostics flag the *other*
                # one still running (e.g. mmdvm_bridge.service left active
                # after switching a BrandMeister config to STFU) instead of
                # just not looking.
                r = subprocess.run(
                    ["systemctl", "is-active", "analog_bridge.service",
                     "mmdvm_bridge.service", "stfu.service"],
                    capture_output=True, text=True, timeout=10)
                ...
                with _dvswitch_status_lock:
                    ...
                    _dvswitch_status_cache["stfu"]          = states[2].strip() if len(states) > 2 else "unknown"
...
# "Who's talking right now" for the Kiosk DVSwitch panel, DVSwitch-
# dashboard-style -- BrandMeister/STFU only for now (see dvswitch/README.md's
# "BrandMeister via STFU" section for why STFU, not MMDVM_Bridge, handles
# BrandMeister here); MMDVM_Bridge's own Homebrew gateway (TGIF/custom) logs
# in a different format this doesn't parse, so those show no caller info.
#
# Tails STFU's own log incrementally (byte offset, not a re-read of the
# whole file each poll) for "ODMR Begin/End Tx: src = <id>, dst = <tg>"
# lines. When the transmitting radio sends DMR Talker Alias data, a
# following "TA = <callsign>" line overrides the ID-lookup callsign with
# what the radio itself broadcast -- more authoritative when present, but
# not every radio/network hop relays it, so it's a nice-to-have, not relied
# on alone.
DVSWITCH_STFU_LOG_PATH = "/var/log/dvswitch/STFU.log"
DVSWITCH_DMRIDS_PATH   = "/var/lib/mmdvm/DMRIds.dat"
_DVSWITCH_BEGIN_TX_RE  = re.compile(r'ODMR Begin Tx: src = (\d+), dst = (\d+)')
...
# single-digit milliseconds per lookup (measured), so loading it wholesale
# would trade a real, permanent RAM cost (this project's Pi Zero 2 W floor
# has 512MB total) for saving an occasional few-ms grep.
_dvswitch_caller_id_cache = {}
        ...
        while True:
            try:
                if os.path.isfile(DVSWITCH_STFU_LOG_PATH):
                    ...
                    if size < offset:
                        offset = 0   # log rotated (logrotate.d/STFU) or truncated
                    with open(DVSWITCH_STFU_LOG_PATH, "r", errors="replace") as f:
                        f.seek(offset)
                        new_data = f.read()
                        offset = f.tell()
