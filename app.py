...
# ---------------------------------------------------------------------------
# Audio monitoring — chunked Ogg/Opus stream via MixMonitor + ffmpeg
# ---------------------------------------------------------------------------
_MONITOR_CONF    = '/etc/asterisk/extensions_asl3ez_monitor.conf'
_MONITOR_CONTEXT = 'asl3ez-monitor'
_audio_lock      = threading.Lock()
_audio_active    = {}   # node -> {proc, fifo, placeholder_fd}
...
def _ensure_monitor_dialplan():
    """Write monitor dialplan context and include it in extensions.conf."""
    changed = False
    try:
...
def _audio_stop(node):
    """Tear down any active audio stream for this node."""
    with _audio_lock:
        state = _audio_active.pop(node, None)
    if not state:
        return
    proc = state.get('proc')
    if proc:
    ...
    if fifo:
        try:
            ...
            pass
    def _hup(ami):
        ami.command(f'channel request hangup Local/{node}@{_MONITOR_CONTEXT};1')
        return {'ok': True}
    try:
...
def api_audio_stream(node):
    ...
    _audio_stop(node)   # clean up any previous stream for this node
    ...
    try:
        ...
        return jsonify({'error': f'FIFO setup failed: {e}'}), 500

    # Open FIFO O_RDWR (non-blocking): holds the write-end open so ffmpeg's
    # blocking O_RDONLY open succeeds and doesn't get premature EOF before
    # MixMonitor connects.  Make it blocking after the open completes.
    try:
        ...
        return jsonify({'error': f'FIFO open failed: {e}'}), 500

    # ffmpeg: read raw slin (s16le 8kHz mono) → encode Ogg/Opus → stdout
    try:
        ...
        return jsonify({'error': f'ffmpeg failed to start: {e}'}), 500

    with _audio_lock:
        _audio_active[node] = {'proc': ffmpeg_proc, 'fifo': fifo_path, 'placeholder_fd': pfd}

    # Originate a Local channel into the monitor dialplan context
    def _originate(ami):
        ami._send_action({
            'Action':      'Originate',
    ...
    try:
        ...
        _audio_stop(node)
        return jsonify({'error': f'AMI originate failed: {e}'}), 500

    def generate():
        try:
            while True:
                chunk = ffmpeg_proc.stdout.read(2048)
                if not chunk:
                    break
                yield chunk
        finally:
