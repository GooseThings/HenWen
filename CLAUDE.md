...
- `start_meshtastic_poller()` — maintains one persistent connection to the public `mqtt.meshtastic.org` broker, subscribed to the owner's configured root topic/channel, caching decoded text messages in-process (`_meshtastic_cache`); re-reads `meshtastic_config` on every reconnect cycle the same way `start_aprs_poller()` does. See "Meshtastic MQTT panel" below.
