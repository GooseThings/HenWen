...
`ntfy`/Pushover/Discord/Kiosk Chat in `alert_config` are independent enable toggles (`ntfy_enabled`/`pushover_enabled`/`discord_enabled`/`chat_enabled`), not a mutually-exclusive single `provider` select like the feature originally shipped with — `_send_alert()` fans an alert out to every enabled destination, each in its own try/except, so one failing (bad token, network hiccup) can't silently swallow the others. Existing installs upgrading from the single-`provider` schema are migrated once, automatically: the old `provider` column is left in the table (unused, harmless) and its value is copied into the matching new `_enabled` column the first time `get_db()` runs after upgrade.
