...
# The actual rule file is generated by provision-sudoers.sh, not inlined
# here, so that update.sh (an in-place upgrade that never re-runs this
# script) can re-run that same file after pulling a new version and pick up
# any new NOPASSWD line a future change adds, instead of only ever getting
# new rules via a manual install.sh re-run.
