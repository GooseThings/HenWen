...
- [x] **FIXED 2026-07-02 — [S2] Secret-key rotation from the UI cannot work under the shipped service config (High, blocks remediation of S1).**
  ...
  **Fix applied:** `install.sh` now installs `/etc/sudoers.d/henwen-systemctl`, a tightly-scoped `NOPASSWD` rule letting `asterisk` run exactly three commands: `systemctl daemon-reload`, `systemctl restart asterisk`, `systemctl restart ASL3-EZ`. A new `_systemctl()` helper in `app.py` runs those through `sudo -n` (fails fast, no password prompt, if the rule is ever missing) and both `api_restart` and `api_set_secret_key` now check the return code and surface a clear, actionable error (pointing at re-running `install.sh`) instead of a generic 500. This resolves **C2** too. Installed and verified live on this node. Writing the unit file itself is still gated by its `600 root:root` permissions (unchanged, intentionally — giving the service account direct write access to its own unit file would be a bigger privilege-escalation surface than the systemctl actions); that failure path already returned a clear `PermissionError` → 403 and its hint was tightened to name the exact manual remediation.

- [x] **FIXED 2026-07-02 — [S3] Manager UI exposed on all interfaces over cleartext HTTP with insecure cookies (High in aggregate).**
  ...
  **Fix applied (scope confirmed with user):** keep `0.0.0.0:5000` for plain-LAN kiosk/manager access — no change to `HOST`, the firewall rule, or the live systemd unit. Instead, `app.py` now installs a `_SchemeAwareSessionInterface` that marks the session cookie `Secure` whenever the *actual* request arrived over HTTPS — including via the local Apache TLS proxy added in `05c7ade`, since `_LocalProxyFix` already restores the real scheme from `X-Forwarded-Proto` for that trusted localhost peer — while still allowing non-Secure cookies over plain LAN HTTP. `SECURE_COOKIES=true` still force-enables Secure-always for HTTPS-only deployments.

- [x] **FIXED 2026-07-02 — [S4] Live audio of any node is streamable without authentication (Med).**
  ...
  **Fix applied:** audio stays public (kiosk use case, confirmed by design). Added `@limiter.limit("30 per minute")` to both `/api/audio/stream/<node>` and `/api/audio/stop`. `_AudioBroadcast` gained a `has_client(remote_addr)` check; `api_audio_stop` now 403s unless the calling IP is a current listener of that node's broadcast, so an anonymous third party can no longer kill someone else's stream.

- [x] **FIXED 2026-07-02 — [S5] `/api/nodestats/batch` does not validate node values before URL interpolation (Low, SSRF-ish, admin-only).**
  ...
  **Fix applied:** each node is now validated with `.isdigit()` before being interpolated; non-numeric entries get `{"error": "Invalid node"}` in the results dict instead of reaching the URL.

- [x] **FIXED 2026-07-02 — [S6] `/logout` is a GET that mutates session state (Low, CSRF).**
  ...
  **Fix applied:** `/logout` is now `methods=["POST"]` (covered by the app's existing global `CSRFProtect`). Updated both frontends: `status.html`'s `kioskLogout()` now POSTs explicitly, and `henwen-manager.html`'s sidebar "Sign out" link is now a button calling a new `doLogout()` that POSTs via `apiFetch` then redirects to `/login`.
...
- [x] **FIXED 2026-07-02 — [C2] Restart/reload features silently ineffective as shipped (Med).** See **S2** — same root cause (non-root service user, no polkit), fixed together via the `henwen-systemctl` sudoers rule + `_systemctl()` helper. `/api/reload` (`asterisk -rx "rpt restart"`) was unaffected by this root cause (it shells out to the `asterisk` CLI binary directly, which the `asterisk` user already owns) and was not touched.
...
- [x] **[A1] FIXED 2026-07-01 — Clicks/pops root cause: `audio_relay.py` had no jitter/accumulation buffer (High for audio quality).**
...
- [x] **[A2] FIXED 2026-07-01 — Secondary pops on live-edge recovery: browser hard-seeked `audio.currentTime` (Med).**
  Fixed in `status.html`: the rate controller now widens the dead-band, adds a smooth 1.25× fast-catch band, and only hard-seeks when >`_LISTEN_JUMP_LAG` (6 s) behind, gated by a 5 s cooldown so lag hovering near the threshold rides the max rate instead of popping repeatedly. Stall recovery now resumes in place and lets the (inaudible, pitch-preserving) rate-catch close the gap, seeking only after an extreme stall. JS syntax-checked. Original analysis retained below.
