...
if [ -f "$CONFIG_JSON" ]; then
  DMR_NETWORK=$(python3 -c "import json,sys; print(json.load(open(sys.argv[1])).get('dmr_network','brandmeister'))" "$CONFIG_JSON" 2>/dev/null || echo "brandmeister")
fi
...
if [ "$DMR_NETWORK" = "brandmeister" ]; then
  if dpkg -s stfu >/dev/null 2>&1; then
    ok "stfu package installed"
  else
    bad "stfu not installed — required for BrandMeister; run guided setup from Manager > DVSwitch"
  fi
fi
...
# apply.sh's own "BrandMeister via STFU, not MMDVM_Bridge" comment for why.
# The unused one being active is flagged, not ignored -- it means Guided
# Setup didn't fully take, or something started it back up by hand.
if [ "$DMR_NETWORK" = "brandmeister" ]; then
  NEEDED_UNIT="stfu"; UNNEEDED_UNIT="mmdvm_bridge"
else
  NEEDED_UNIT="mmdvm_bridge"; UNNEEDED_UNIT="stfu"
fi
for unit in analog_bridge "$NEEDED_UNIT"; do
...
state=$(systemctl is-active "${UNNEEDED_UNIT}.service" 2>/dev/null || echo "inactive")
if [ "$state" != "active" ]; then
  ok "${UNNEEDED_UNIT}.service correctly inactive (not used for $DMR_NETWORK)"
else
  warn "${UNNEEDED_UNIT}.service is active but isn't the path used for $DMR_NETWORK -- guided setup should have stopped it; re-run it or stop it by hand"
fi

echo "== Analog_Bridge routing"
EXPECTED_MODE="DMR"; [ "$DMR_NETWORK" = "brandmeister" ] && EXPECTED_MODE="STFU"
ACTUAL_MODE=$("$DVSWITCH_SH" show 2>/dev/null | python3 -c "import json,sys; print(json.load(sys.stdin).get('tlv',{}).get('ambe_mode',''))" 2>/dev/null || echo "")
if [ "$ACTUAL_MODE" = "$EXPECTED_MODE" ]; then
  ok "Analog_Bridge is routed to $EXPECTED_MODE"
elif [ -z "$ACTUAL_MODE" ]; then
  warn "Could not read Analog_Bridge's current mode -- is analog_bridge.service running?"
else
  bad "Analog_Bridge is routed to $ACTUAL_MODE, expected $EXPECTED_MODE for $DMR_NETWORK -- run: $DVSWITCH_SH mode $EXPECTED_MODE"
fi
...
if [ -f "$ANALOG_BRIDGE_INI" ]; then
  # Section-scoped and comment-stripping (a plain grep for "^txPort\s*="
  # anywhere in the file matches [AMBE_AUDIO]'s txPort too, which comes
  # first in the file -- confirmed live this silently reported the wrong
  # port's bind state). The dummy leading section (same trick dvswitch.sh's
  # own parseIniFile() uses) works around Analog_Bridge.ini having an
  # `include = ...` directive before its first real [section] header,
  # which configparser otherwise rejects outright.
  read_usrp_port() {
    python3 -c "
import configparser
c = configparser.RawConfigParser(inline_comment_prefixes=(';',))
with open('$ANALOG_BRIDGE_INI') as f:
    c.read_string('[dummy]\n' + f.read())
print(c.get('USRP', '$1', fallback='').strip())
" 2>/dev/null
  }
  TXPORT=$(read_usrp_port txPort)
  RXPORT=$(read_usrp_port rxPort)
  for p in "$TXPORT" "$RXPORT"; do
    [ -n "$p" ] || continue
    if ss -uln 2>/dev/null | grep -q ":${p} "; then
      ok "UDP ${p} bound (Analog_Bridge)"
    else
      warn "UDP ${p} not currently bound — normal if analog_bridge.service just started; re-check if this persists"
    fi
  done
else
